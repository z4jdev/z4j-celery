...
logger = logging.getLogger("z4j.agent.celery.broker_events")
...
class CeleryBrokerEventsMonitor:
    ...
    def _run(self) -> None:
        ...
        # which breaks our redaction-aware mapper. ``task-received``
        # fires immediately after ``task-sent`` and gives us the same
        # information in a usable shape.
        ...
        def _dispatch(body: Any, message: Any) -> None:
            ...
            """
            # Audit H11: ack ONLY after the handler succeeded. The
            ...
            try:
                with self._app.connection_for_read() as conn:
                    ...
                    consumer = conn.Consumer(
                        queues=[recv.queue],
                        callbacks=[_dispatch],
                        # Audit H11: was ``no_ack=True`` (lost
